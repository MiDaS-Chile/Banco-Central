...
<p align = "justify">
**Nota:** Solo se pueden crear variables con caractéres alfanuméricos, "." y "_". Los espacios no están permitidos.</p>
...
Se puede acceder a un elemento de un vector en una posición específica de un vector utilizando '[]'. Algunos ejemplos:
...
- Una herramienta para la manipulación de objetos son las funciones. Una función en `R` tiene la estructura de `nombre_funcion(<lista de argumentos>)`.
...
<p align = "justify">
Los paquetes de R son el eje central de su funcionamiento. Cada uno de ellos provee de diferentes funciones desarrolladas para resolver diferentes tipos de problemáticas. En esta ocasión instalaremos y cargaremos el paquete 'Tidyverse': </p>
...
<p align = "justify">
Permite realizar composición de funciones. Un ejemplo de su utilización es: </p>
...
- Si es spss: **read_sav**.
- Para más información, pueden entrar al siguiente [enlace](https://resources.rstudio.com/spanish-pdfs/data-import-cheatsheet-spanish)
...
Permite seleccionar filas de cierta base de datos utilizando un criterio particular.
...
<p align = "justify">
Permite ordenar de menor a mayor una base de datos teniendo en consideración una o más variables. </p>
...
Permite crear o modificar una columna de la base de datos.
...
## mutate + case_when: *Indicadores*

<p align = "justify">
`case_when()` es una función que permite, en conjunto con `mutate`, generar variables indicadoras. </p>
...
<p align = "justify">
Existen 7 funciones, en dplyr, que permiten juntar/contrastar dos bases de datos en una sola, utilizando una columna como link.</p> 
...
## inner_join y right_join : *Código*
...
¿Qué pasa si las columnas 'links' poseen nombres diferentes?
...
- Para más gráficos, pueden visitar el siguiente [sitio web.](http://www.sthda.com/english/wiki/be-awesome-in-ggplot2-a-practical-guide-to-be-highly-effective-r-software-and-data-visualization)
...
Quizás en lugar de asignar color fijo, podemos asignarlo segun una variable
...
## Paquete Rmisc: *Gráficos en la misma ventana*
...
<p align = "justify"> Instale los siguientes paquetes: `babynames`, `nycflights13`, `maps` y carguelos (con `library`) junto a `tidyverse`. Resuelva las siguientes preguntas: </p>

<p align = "justify"> **Pregunta 1** </p>

<p align = "justify"> Considere la tabla `births` paquete `babynames`.</p>
...
<p align = "justify"> **Pregunta 2** </p>

<p align = "justify"> Considere ahora la tabla `babynames` del paquete `babynames`. </p>

- Obtener la misma estructura en cuanto a **columnas** que la tabla `births`, esto es, generar una nueva tabla que nos entregue el número de nacimientos por año.
- Genere una tabla partir de `babynames` que contenga los nacimientos de las personas de género femenino con el nombre **Nala, Ariel** y **Elsa** desde los años 1980.
- Con el `tibble` anterior genere un gráfico que contenga la información de los nacimientos por año de cada uno de los nombres mencionados. *Hint:* Use `face_wrap(~ name, scales = "free_y")`.

----

<p align = "justify"> **Pregunta 3** </p>

<p align = "justify"> Utilizando las tablas `airports` y `flights` del paquete `nycflights13`</p>

- Obtenga una tabla que contenga conteos de vuelos según su destino `dest` además de la longitud y latitud del aeropuerto(de destino). 

----

<p align = "justify"> **Pregunta 4** </p>

<p align = "justify"> Considere el siguiente gráfico </p>
...
- Agregue una capa de puntos ubicando los aeropuertos obtenidos de la pregunta anterior y usando además: `aes(size = la_cantidad_de_vuelos_a_dicho_aeropuerto)`.
- A la izquierda del gráfico anterior se observan 2 puntos. Genere el/los pasos necesarios para seleccionarlos usando la tabla resultante de la pregunta 5 para identificar los nombres de dichos aeropuertos y responda: ¿Donde están? ¿Qué gracias tienen?
